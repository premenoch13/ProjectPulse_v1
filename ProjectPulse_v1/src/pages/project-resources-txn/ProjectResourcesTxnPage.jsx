...
export function ProjectResourcesTxnPage() {
  ...
  const toggleActive = (r0) => {
    ...
    callProjectResourceTxnFlow("EDIT", { ...r0, active: !r0.active })
      .then((res) => {
        setRows(res.data);
        logAudit("Project Resources", "Update", `${userName(r0.userId)} — ${projectName(r0.projectId)}`);
  ...
  return (
    <div style={{ flex: 1, display: "flex", overflow: "hidden", position: "relative" }}>
      ...
      {panel && (
        <ProjectResourcesTxnPanel mode={panel.mode} data={panel.data} projects={projects} users={users} roles={roles} saving={saving} error={err} onCancel={() => {
            if (panel?.mode === "add") {
