...
import { callRoleFlow, checkReferences } from "../../api/flows";
...
export function RolesPage() {
  ...
  const submitPanel = (form) => {
    ...
    if (findDuplicateName(rows, form.name, form.guid)) {
      setErr("Role Name already exists. Please enter a unique Role Name.");
      return;
    }
    setSaving(true);
    setErr("");
  ...
  const toggleActive = (r0) => {
    ...
    callRoleFlow("EDIT", { ...r0, active: !r0.active })
      .then((res) => {
        setRows(res.data);
        logAudit("Role", "Update", r0.name || r0.code || "record");
  ...
  const openEdit = async (r) => {
    const guard = await checkReferences("Role", r.guid);
    if (guard.blocked) {
  ...
  const confirmDeleteRow = async () => {
    ...
    const guard = await checkReferences("Role", confirmDelete.guid);
    if (guard.blocked) {
      setToast(guard.message);
    ...
    <div style={{ flex: 1, display: "flex", overflow: "hidden", position: "relative" }}>
      <div style={{ flex: 1, padding: 26, overflowY: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 18 }}>
          <div>
            <div style={{ fontFamily: "Sora, sans-serif", fontSize: 20, fontWeight: 700, color: COLORS.text }}>Role</div>
