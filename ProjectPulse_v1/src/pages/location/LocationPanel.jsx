...
export function LocationPanel({ mode, data, countries, saving, error, codeLocked, onCancel, onClose, onSubmit }) {
  ...
  return (
    <div className="pp-panel" style={{
      ...
      <div style={{ padding: "18px 20px", borderBottom: `1px solid ${COLORS.border}`, display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          <div style={{ fontWeight: 700, fontSize: 15, color: COLORS.text }}>{mode === "add" ? "Add New Location" : "Edit Location"}</div>
      ...
      <div style={{ padding: 20, flex: 1, overflowY: "auto" }}>
        ...
        {codeLocked && (
          <div style={{ fontSize: 11.5, color: COLORS.textMuted, marginTop: 4 }}>
            Code is locked — this location is already in use elsewhere.
          </div>
        )}
        <label style={{ ...labelStyle, marginTop: 16 }}>Location Name*</label>
        <input
          value={form.name}
