...
import { callDesignationFlow, checkReferences } from "../../api/flows";
...
export function DesignationPage() {
  ...
  const [confirmDelete, setConfirmDelete] = useState(null); // null | designation row
  ...
  const refresh = useCallback(() => {
    setLoading(true);
    setListError("");
    callDesignationFlow("LIST").then((res) => {
      setRows(res.data);
      setLoading(false);
    }).catch((e) => {
  ...
  const kpis = [
    { label: "Total Designations", value: String(rows.length), icon: Tags, color: "#6366F1" },
    { label: "Active Designations", value: String(activeCount), icon: CheckCircle2, color: COLORS.success },
    { label: "Inactive Designations", value: String(rows.length - activeCount), icon: AlertCircle, color: COLORS.danger },
  ];

  const submitPanel = (form) => {
    if (!form.code?.trim() || !form.name?.trim()) {
      setErr("Designation Code and Name are required.");
      return;
    }
    if (findDuplicateCode(rows, form.code, form.guid)) {
      setErr("Designation Code already exists.");
      return;
    }
    if (findDuplicateName(rows, form.name, form.guid)) {
      setErr("Designation Name already exists. Duplicate Designations are not allowed.");
      return;
    }
    setSaving(true);
    setErr("");
    const action = form.guid ? "EDIT" : "CREATE";
    callDesignationFlow(action, form)
      .then((res) => {
        setRows(res.data); // flow returns the refreshed list — reflect it immediately
        setSaving(false);
        setPanel(null);
        logAudit("Designation", form.guid ? "Update" : "Create", form.name || form.code || form.empId || form.contactName || "record");
        setToast(form.guid ? "Designation updated." : "Designation added.");
      })
      .catch((e) => {
        setSaving(false);
        setErr(e.message);
  ...
  const toggleActive = (d) => {
    ...
    callDesignationFlow("EDIT", { ...d, active: !d.active })
      .then((res) => {
        setRows(res.data);
        logAudit("Designation", "Update", d.name || d.code || "record");
  ...
  const openEdit = async (d) => {
    const guard = await checkReferences("Designation", d.guid);
    setPanel({ mode: "edit", data: { ...d }, codeLocked: guard.blocked });
  };

  const confirmDeleteRow = async () => {
    if (!confirmDelete) return;
    const guard = await checkReferences("Designation", confirmDelete.guid);
    if (guard.blocked) {
    ...
    setDeleting(true);
    callDesignationFlow("DELETE", confirmDelete)
      .then((res) => {
        setRows(res.data);
        setDeleting(false);
        setConfirmDelete(null);
        logAudit("Designation", "Delete", confirmDelete.name || confirmDelete.code || confirmDelete.empId || confirmDelete.contactName || "record");
        setToast("Designation deleted.");
      })
      .catch((e) => {
        setDeleting(false);
      ...
      <div style={{ flex: 1, padding: 26, overflowY: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 18 }}>
          <div>
            ...
            <div style={{ color: COLORS.textMuted, fontSize: 13.5 }}>Add, edit and manage Designations</div>
          </div>
          <button
        ...
        <div style={{ ...cardStyle, padding: 0, overflow: "hidden" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px", borderBottom: `1px solid ${COLORS.border}` }}>
            <div style={{ fontWeight: 700, fontSize: 14, color: COLORS.text }}>Designations <span style={{ color: COLORS.textMuted, fontWeight: 500 }}>({filtered.length})</span></div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: COLORS.bg }}>
                {["Designation Code", "Designation Name", "Status","Actions"].map((h) => (
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={4} style={{ padding: 40, textAlign: "center", color: COLORS.textMuted }}>
                  <Loader2 size={18} className="spin" style={{ verticalAlign: "middle", marginRight: 8 }} /> Loading designations…
                </td></tr>
              ) : listError ? (
