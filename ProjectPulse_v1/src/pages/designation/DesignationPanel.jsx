...
export function DesignationPanel({ mode, data, saving, error, codeLocked, onCancel, onClose, onSubmit }) {
  ...
  return (
    <div className="pp-panel" style={{
      ...
      <div style={{ padding: "18px 20px", borderBottom: `1px solid ${COLORS.border}`, display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          <div style={{ fontWeight: 700, fontSize: 15, color: COLORS.text }}>{mode === "add" ? "Add New Designation" : "Edit Designation"}</div>
      ...
      <div style={{ padding: 20, flex: 1, overflowY: "auto" }}>
        <label style={labelStyle}>Designation Code*</label>
        <input
          value={form.code}
          onChange={(e) => setForm({ ...form, code: e.target.value })}
          placeholder="Enter designation code"
          disabled={codeLocked}
          style={{ ...inputStyle, ...(codeLocked ? { background: "#F3F4F6", color: COLORS.textMuted, cursor: "not-allowed" } : {}) }}
        />
        {codeLocked && (
          <div style={{ fontSize: 11.5, color: COLORS.textMuted, marginTop: 4 }}>
            Code is locked — this designation is already in use elsewhere.
          </div>
        )}
        <label style={{ ...labelStyle, marginTop: 16 }}>Designation Name*</label>
        <input
          value={form.name}
