...
import { callApprovalStatusFlow } from "../../api/flows";
...
export function ApprovalStatusPage() {
  ...
  const submitPanel = (form) => {
    ...
    if (findDuplicateCode(rows, form.code, form.guid)) {
      setErr("Approval Status Code already exists. Please enter a unique Code.");
      return;
    }
    if (findDuplicateName(rows, form.name, form.guid)) {
      setErr("Approval Status Name already exists. Please enter a unique Name.");
      return;
    }
    setSaving(true);
    setErr("");
    ...
    <div style={{ flex: 1, display: "flex", overflow: "hidden", position: "relative" }}>
      <div style={{ flex: 1, padding: 26, overflowY: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 18 }}>
          <div>
            <div style={{ fontFamily: "Sora, sans-serif", fontSize: 20, fontWeight: 700, color: COLORS.text }}>Approval Status</div>
      ...
      {panel && (
        <ApprovalStatusPanel mode={panel.mode} data={panel.data} saving={saving} error={err} onCancel={() => {
            if (panel?.mode === "add") {
