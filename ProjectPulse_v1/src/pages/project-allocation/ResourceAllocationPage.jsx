...
export function ResourceAllocationPage() {
  ...
  const submitPanel = (form) => {
    ...
    callProjectResourceFlow(action, form)
      ...
      .then(() => {
        setSaving(false);
        setPanel(null);
        logAudit("Resource Allocation", form.guid ? "Update" : "Create", `${userLabel(lookups.users, form.userId)} → ${projects.find((p) => String(p.id) === String(form.projectId))?.projectName || "project"}`);
  ...
  const toggleBillable = (a0) => {
    callProjectResourceFlow("EDIT", { ...a0, billable: !a0.billable })
      .then(() => refresh())
      .then(() => {
        logAudit("Resource Allocation", "Update", `${userLabel(lookups.users, a0.userId)} → ${a0.projectName}`);
  ...
  return (
    <div style={{ flex: 1, display: "flex", overflow: "hidden", position: "relative" }}>
            ...
            <table style={{ width: "100%", minWidth: 1080, borderCollapse: "collapse" }}>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={10}><EmptyState onRetry={refresh} /></td></tr>
                ) : filtered.length === 0 ? (
                  <tr><td colSpan={10} style={{ padding: 40, textAlign: "center", color: COLORS.textMuted }}>No data available.</td></tr>
                ) : filtered.map((a, i) => (
                  <tr key={a.id} style={{ borderTop: `1px solid ${COLORS.border}`, background: i % 2 ? "#FAFBFD" : "#fff" }}>
                    ...
                    <td style={{ padding: "11px 12px", fontSize: 12, color: COLORS.textMuted, whiteSpace: "nowrap" }}>{fmtDate(a.startDate)} → {fmtDate(a.endDate)}</td>
      ...
      {panel && <AllocationPanel data={panel} saving={saving} error={err} onCancel={() => { openAdd(); setErr(""); }} onClose={() => setPanel(null)}
