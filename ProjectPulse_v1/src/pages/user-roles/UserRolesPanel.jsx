...
export function UserRolesPanel({ mode, data, users, roles, saving, error, onCancel, onClose, onSubmit }) {
    ...
    <div className="pp-panel" style={{ width: 340, background: COLORS.card, borderLeft: `1px solid ${COLORS.border}`, flexShrink: 0, display: "flex", flexDirection: "column", boxShadow: "-8px 0 30px rgba(15,20,40,0.06)" }}>
      ...
      <div style={{ padding: 20, flex: 1, overflowY: "auto" }}>
        ...
        <select value={form.userId || ""} onChange={(e) => setForm({ ...form, userId: e.target.value })} style={inputStyle}>
          ...
          {activeOptions(users, form.userId).map((u) => (
        ...
        <select value={form.roleId || ""} onChange={(e) => setForm({ ...form, roleId: e.target.value })} style={inputStyle}>
          ...
          {activeOptions(roles, form.roleId).map((r) => (
