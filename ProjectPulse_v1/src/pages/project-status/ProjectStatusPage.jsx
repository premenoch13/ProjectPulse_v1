...
import { callProjectStatusFlow } from "../../api/flows";
...
export function ProjectStatusPage() {
  ...
  const submitPanel = (form) => {
    ...
    if (findDuplicateName(rows, form.name, form.guid)) {
      setErr("Project Status Name already exists. Please enter a unique Name.");
      return;
    }
    setSaving(true);
    setErr("");
  ...
  const toggleActive = (r0) => {
    ...
    callProjectStatusFlow("EDIT", { ...r0, active: !r0.active })
      .then((res) => {
        setRows(res.data);
        logAudit("Project status", "Update", r0.name || r0.code || "record");
    ...
    <div style={{ flex: 1, display: "flex", overflow: "hidden", position: "relative" }}>
      <div style={{ flex: 1, padding: 26, overflowY: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 18 }}>
          <div>
            <div style={{ fontFamily: "Sora, sans-serif", fontSize: 20, fontWeight: 700, color: COLORS.text }}>Project Status</div>
      ...
      {panel && (
        <ProjectStatusPanel mode={panel.mode} data={panel.data} saving={saving} error={err} onCancel={() => {
            if (panel?.mode === "add") {
