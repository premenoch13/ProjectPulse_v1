...
export function BillingTypePage() {
  ...
  const submitPanel = (form) => {
    ...
    if (findDuplicateName(rows, form.name, form.guid)) {
      setErr("Billing Type Name already exists. Please enter a unique Billing Type Name.");
      return;
    }
    setSaving(true);
    setErr("");
  ...
  const toggleActive = (r0) => {
    ...
    callBillingTypeFlow("EDIT", { ...r0, active: !r0.active })
      .then((res) => {
        setRows(res.data);
        logAudit("Billing type", "Update", r0.name || r0.code || "record");
  ...
  const openEdit = async (r) => {
    const guard = await checkReferences("BillingType", r.guid);
    if (guard.blocked) {
      setToast(guard.message);
      return;
    }
    setPanel({ mode: "edit", data: { ...r } });
  };

  const confirmDeleteRow = async () => {
    if (!confirmDelete) return;
    const guard = await checkReferences("BillingType", confirmDelete.guid);
    if (guard.blocked) {
      setToast(guard.message);
    ...
    <div style={{ flex: 1, display: "flex", overflow: "hidden", position: "relative" }}>
      <div style={{ flex: 1, padding: 26, overflowY: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 18 }}>
          <div>
            <div style={{ fontFamily: "Sora, sans-serif", fontSize: 20, fontWeight: 700, color: COLORS.text }}>Billing Type</div>
