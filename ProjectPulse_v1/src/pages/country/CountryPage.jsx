...
import { callCountryFlow, callCurrencyFlow, checkReferences } from "../../api/flows";
...
const EMPTY_FORM = { guid: "", code: "", name: "", currencyId: "", active: true };
...
export function CountryPage() {
  ...
  const submitPanel = (form) => {
    ...
    if (findDuplicateCode(rows, form.code, form.guid)) {
      setErr("Country Code already exists. Please enter a unique Country Code.");
      return;
    }
    if (findDuplicateName(rows, form.name, form.guid)) {
      setErr("Country Name already exists. Please enter a unique Country Name.");
      return;
    }
    setSaving(true);
    setErr("");
    ...
    callCountryFlow(action, form)
      .then((res) => {
        ...
        setSaving(false);
        setPanel(null);
        logAudit("Country", form.guid ? "Update" : "Create", form.name || form.code || "record");
  ...
  const toggleActive = (c) => {
    setRows((prev) => prev.map((r) => (r.guid === c.guid ? { ...r, _toggling: true } : r)));
    callCountryFlow("EDIT", { ...c, active: !c.active })
      .then((res) => {
        setRows(res.data);
        logAudit("Country", "Update", c.name || c.code || "record");
        setToast(`Marked ${!c.active ? "Active" : "Inactive"}.`);
      })
      .catch((e) => {
        setRows((prev) => prev.map((r) => (r.guid === c.guid ? { ...r, _toggling: false } : r)));
        setToast(`Update failed: ${e.message}`);
      });
  };

  const openEdit = async (c) => {
    const guard = await checkReferences("Country", c.guid);
    if (guard.blocked) {
  ...
  const confirmDeleteRow = async () => {
    ...
    const guard = await checkReferences("Country", confirmDelete.guid);
    if (guard.blocked) {
      setToast(guard.message);
    ...
    callCountryFlow("DELETE", confirmDelete)
      .then((res) => {
        ...
        setDeleting(false);
        setConfirmDelete(null);
        logAudit("Country", "Delete", confirmDelete.name || confirmDelete.code || "record");
    ...
    <div style={{ flex: 1, display: "flex", overflow: "hidden", position: "relative" }}>
      <div style={{ flex: 1, padding: 26, overflowY: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 18 }}>
          <div>
            <div style={{ fontFamily: "Sora, sans-serif", fontSize: 20, fontWeight: 700, color: COLORS.text }}>Country</div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: COLORS.bg }}>
                {["Country Code", "Country Name", "Currency", "Status", "Actions"].map((h) => (
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={5} style={{ padding: 40, textAlign: "center", color: COLORS.textMuted }}>No data available.</td></tr>
              ) : filtered.map((c, i) => (
                <tr key={c.id} style={{ borderTop: `1px solid ${COLORS.border}`, background: i % 2 ? "#FAFBFD" : "#fff" }}>
                  ...
                  <td style={{ padding: "11px 16px", fontSize: 13.5, color: COLORS.text }}>{currencyLabel(c.currencyId)}</td>
                  <td style={{ padding: "11px 16px" }}><StatusBadge active={c.active} busy={c._toggling} onToggle={() => toggleActive(c)} /></td>
                  <td style={{ padding: "11px 16px", textAlign: "center" }}>
                    <div style={{ display: "inline-flex", gap: 8 }}>
                      <button onClick={() => openEdit(c)} style={{ display: "inline-flex", alignItems: "center", gap: 6, background: COLORS.accentSoft, color: COLORS.accent, border: "none", borderRadius: 7, padding: "6px 11px", fontSize: 12.5, fontWeight: 600, cursor: "pointer" }}>
                        <Pencil size={12} /> Edit
                      </button>
                      <button onClick={() => setConfirmDelete(c)} style={{ display: "inline-flex", alignItems: "center", gap: 6, background: COLORS.dangerSoft, color: COLORS.danger, border: "none", borderRadius: 7, padding: "6px 11px", fontSize: 12.5, fontWeight: 600, cursor: "pointer" }}>
