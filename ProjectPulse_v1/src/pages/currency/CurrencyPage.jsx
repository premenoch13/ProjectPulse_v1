...
import { callCurrencyFlow, callCountryFlow, checkReferences } from "../../api/flows";
...
export function CurrencyPage() {
  ...
  const [confirmDelete, setConfirmDelete] = useState(null); // null | currency row
  ...
  const refresh = useCallback(() => {
    setLoading(true);
    setListError("");
    callCurrencyFlow("LIST").then((res) => {
      setRows(res.data);
      setLoading(false);
    }).catch((e) => {
  ...
  const kpis = [
    { label: "Total Currencies", value: String(rows.length), icon: Tags, color: "#6366F1" },
    { label: "Active Currencies", value: String(activeCount), icon: CheckCircle2, color: COLORS.success },
    { label: "Inactive Currencies", value: String(rows.length - activeCount), icon: AlertCircle, color: COLORS.danger },
  ];

  const submitPanel = (form) => {
    if (!form.code?.trim() || !form.name?.trim()) {
      setErr("Currency Code and Name are required.");
      return;
    }
    if (findDuplicateCode(rows, form.code, form.guid)) {
      setErr("Currency Code already exists.");
      return;
    }
    if (findDuplicateName(rows, form.name, form.guid)) {
      setErr("Currency Name already exists. Please enter a unique Currency Name.");
      return;
    }
    setSaving(true);
    setErr("");
    const action = form.guid ? "EDIT" : "CREATE";
    callCurrencyFlow(action, form)
      .then((res) => {
        setRows(res.data); // flow returns the refreshed list — reflect it immediately
        setSaving(false);
        setPanel(null);
        logAudit("Currency", form.guid ? "Update" : "Create", form.name || form.code || form.empId || form.contactName || "record");
        setToast(form.guid ? "Currency updated." : "Currency added.");
      })
      .catch((e) => {
        setSaving(false);
        setErr(e.message);
  ...
  const toggleActive = (d) => {
    ...
    callCurrencyFlow("EDIT", { ...d, active: !d.active })
      .then((res) => {
        setRows(res.data);
        logAudit("Currency", "Update", d.name || d.code || "record");
  ...
  const openEdit = async (d) => {
    const guard = await checkReferences("Currency", d.guid);
    setPanel({ mode: "edit", data: { ...d }, codeLocked: guard.blocked });
  };

  const confirmDeleteRow = async () => {
    if (!confirmDelete) return;
    const guard = await checkReferences("Currency", confirmDelete.guid);
    if (guard.blocked) {
    ...
    setDeleting(true);
    callCurrencyFlow("DELETE", confirmDelete)
      .then((res) => {
        setRows(res.data);
        setDeleting(false);
        setConfirmDelete(null);
        logAudit("Currency", "Delete", confirmDelete.name || confirmDelete.code || confirmDelete.empId || confirmDelete.contactName || "record");
        setToast("Currency deleted.");
      })
      .catch((e) => {
        setDeleting(false);
    ...
    <div style={{ flex: 1, display: "flex", overflow: "hidden", position: "relative" }}>
      <div style={{ flex: 1, padding: 26, overflowY: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 18 }}>
          <div>
            <div style={{ fontFamily: "Sora, sans-serif", fontSize: 20, fontWeight: 700, color: COLORS.text }}>Currency</div>
            <div style={{ color: COLORS.textMuted, fontSize: 13.5 }}>Add, edit and manage Currencies</div>
          </div>
          <button
            onClick={() => setPanel({ mode: "add", data: { guid: "", code: "", name: "", symbol: "", countryId: "", active: true } })}
            style={{
        ...
        <div style={{ ...cardStyle, padding: 0, overflow: "hidden" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px", borderBottom: `1px solid ${COLORS.border}` }}>
            <div style={{ fontWeight: 700, fontSize: 14, color: COLORS.text }}>Currencies <span style={{ color: COLORS.textMuted, fontWeight: 500 }}>({filtered.length})</span></div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: COLORS.bg }}>
                {["Currency Code", "Currency Name", "Status","Actions"].map((h) => (
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={4} style={{ padding: 40, textAlign: "center", color: COLORS.textMuted }}>
                  <Loader2 size={18} className="spin" style={{ verticalAlign: "middle", marginRight: 8 }} /> Loading currencies…
                </td></tr>
              ) : listError ? (
      ...
      {panel && (
        <CurrencyPanel
          ...
          onCancel={() => {
            if (panel?.mode === "add") {
              setPanel({ mode: "add", data: { guid: "", code: "", name: "", symbol: "", countryId: "", active: true } });
            } else {
              setPanel(null);
            }
            setErr("");
