...
import { callInvoiceStatusFlow } from "../../api/flows";
...
export function InvoiceStatusPage() {
  ...
  const submitPanel = (form) => {
    ...
    if (findDuplicateName(rows, form.name, form.guid)) {
      setErr("Invoice Status Name already exists. Please enter a unique Invoice Status Name.");
      return;
    }
    setSaving(true);
    setErr("");
  ...
  setRows((prev) =>
    prev.map((r) =>
      r.guid === r0.guid ? { ...r, _toggling: true } : r
    )
  );

  callInvoiceStatusFlow("EDIT", {
    ...r0,
    active: !r0.active,
  })
    .then((res) => {
    ...
    .catch((e) => {
      setRows((prev) =>
        prev.map((r) =>
          r.guid === r0.guid ? { ...r, _toggling: false } : r
        )
      );
    ...
    <div style={{ flex: 1, display: "flex", overflow: "hidden", position: "relative" }}>
      <div style={{ flex: 1, padding: 26, overflowY: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 18 }}>
          <div>
            <div style={{ fontFamily: "Sora, sans-serif", fontSize: 20, fontWeight: 700, color: COLORS.text }}>Invoice Status</div>
...
<td style={{ padding: "11px 16px" }}>
  ...
  />
</td>                  <td style={{ padding: "11px 16px", textAlign: "center" }}>
      ...
      {panel && (
        <InvoiceStatusPanel mode={panel.mode} data={panel.data} saving={saving} error={err} onCancel={() => {
            if (panel?.mode === "add") {
