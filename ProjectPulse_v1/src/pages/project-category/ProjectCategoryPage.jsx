...
import { callProjectCategoryFlow, checkReferences } from "../../api/flows";
import { ConfirmModal } from "../../components/common/ConfirmModal";
import { StatusBadge } from "../../components/common/StatusBadge";
import { COLORS, cardStyle } from "../../constants/theme";
import { ProjectCategoryPanel } from "./ProjectCategoryPanel";
...
export function ProjectCategoryPage() {
  ...
  const submitPanel = (form) => {
    ...
    if (findDuplicateName(rows, form.name, form.guid)) {
      setErr("Category Name already exists. Please enter a unique Category Name.");
      return;
    }
    setSaving(true);
    setErr("");
  ...
  const toggleActive = (c) => {
    ...
    callProjectCategoryFlow("EDIT", { ...c, active: !c.active })
      .then((res) => {
        setRows(res.data);
        logAudit("Category", "Update", c.name || c.code || "record");
  ...
  const openEdit = async (c) => {
    const guard = await checkReferences("ProjectCategory", c.guid);
    setPanel({ mode: "edit", data: { ...c }, codeLocked: guard.blocked });
  };

  const confirmDeleteRow = async () => {
    if (!confirmDelete) return;
    const guard = await checkReferences("ProjectCategory", confirmDelete.guid);
    if (guard.blocked) {
    ...
    <div style={{ flex: 1, display: "flex", overflow: "hidden", position: "relative" }}>
      <div style={{ flex: 1, padding: 26, overflowY: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 18 }}>
          <div>
            <div style={{ fontFamily: "Sora, sans-serif", fontSize: 20, fontWeight: 700, color: COLORS.text }}>Project Category</div>
